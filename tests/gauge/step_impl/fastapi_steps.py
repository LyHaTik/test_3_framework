...
def timed_request(name, method, url, **kwargs):
    """Выполнить HTTP-запрос с измерением времени и выводом таблицы результатов"""
    start = time.perf_counter()
    resp = requests.request(method, url, **kwargs)
    elapsed = time.perf_counter() - start
    record_result("FastAPI", name, elapsed, resp.status_code)
    print(f"{name} | {method.upper()} | {url} | {elapsed:.3f}s | {resp.status_code}")
    return resp


@step("Run full CRUD scenario for FastAPI API <table>")
def run(table):
    if not table.rows:
